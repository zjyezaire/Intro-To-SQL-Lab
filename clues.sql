...
WHERE code IN (
    SELECT countrycode
    FROM countrylanguages
    WHERE language = 'LanguageFromClue2'
...
WHERE countrycode = (
    SELECT code
    FROM countries
    WHERE name = 'NameFromClue3'
)
AND name NOT LIKE '%NameFromClue3%';
...
FROM cities
WHERE name LIKE 'PartialNameFromClue4%'
AND name NOT LIKE 'ExactNameFromClue4%';
...
-- Lucky for us, she's getting cocky. She left us a note (below), and I'm sure she thinks she's very clever, but if we can crack it,
